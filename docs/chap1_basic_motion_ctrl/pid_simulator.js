...
function initializePIDSimulator(containerId, showZieglerNicholsButton = false) {
    ...
    function updateChart() {
        ...
        const pidSystem = new SecondOrderSystem();
        pidSystem.enableNoise(noiseEnabled);
    ...
    let noiseEnabled = false; // Global flag inside initializePIDSimulator

    const noiseToggleButton = container.querySelector('.noise-toggle-button');
    if (noiseToggleButton) {
        noiseToggleButton.addEventListener('click', () => {
            noiseEnabled = !noiseEnabled;
            noiseToggleButton.textContent = noiseEnabled ? 'Noise: ON' : 'Noise: OFF';
            updateChart(); // Refresh chart with noise setting
        });
    }
