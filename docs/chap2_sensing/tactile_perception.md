...
- Basics of mechanics (normal and shear stress)
...
<h4 class="section-title">Basic capacitive sensor:</h4>
...
<figure style="text-align: center;">
  ...
  <div style="display: flex; justify-content: center; gap: 20px;">
    <div style="flex: 1;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezoelectric_effect_pvdf_shrunk.jpeg' }}"
           width="300px"
    ...
    <div style="flex: 1;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezoelectric_effect_pvdf_stretched.jpeg' }}"
           width="240px"
...
A characteristic of these sensors is that they are primarily suited for **dynamic sensing**, they detect changes in force ($\frac{dF}{dt}$). If a constant load is maintained, the generated electrical charge decays to zero. This means piezoelectric sensors are excellent at detecting **vibrations** or the exact moment of initial contact, but they cannot sense a steady weight over a long period.
...
* **$d$** is the piezoelectric constant of the material and
* **$F$** is the applied Force.

In our simplified formula, we treat $d$ as a single value and $F$ as a uniaxial force. However, in reality, $d$ is a **tensor**. In the drop-down below, you can see what this tensor looks like.
...
<details class="optional-details" markdown="1">
  ...
  <div class="optional-window">
    <p>
      Because piezoelectric materials are crystalline or polarized polymers, their electrical response depends on the <strong>orientation</strong> of the applied mechanical stress relative to the material's axes. Below is a schematic as a reminder of the notation for mechanical stress, including normal stress ($\sigma_{11}, \sigma_{22}, \sigma_{33}$) and shear stress ($\sigma_{12}, \sigma_{23}, \sigma_{31}$).
    </p>

    <figure style="text-align: center;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_mechanical_stress_notation.png' }}"
           width="360px"
           alt="Mechanical stress notation on a cubic element">
      <figcaption>
        <sub><i>
    ...
    <p>
      The relationship between the <strong>generated charge</strong> and the <strong>applied mechanical stress</strong> $\boldsymbol{\sigma} = (\sigma_{11}, \sigma_{22}, \sigma_{33}, \sigma_{23}, \sigma_{31}, \sigma_{12})^T$ is defined by the piezoelectric coefficient matrix $d$ ($3 \times 6$). This matrix maps the six stress components to the three electrical axes ($x_1, x_2, x_3$).
    </p>
    ...
    <p>
      The piezoelectric relationship becomes:
    </p>

    \[
    \mathbf{Q} = d \cdot \boldsymbol{\sigma} = \begin{pmatrix} 
    0 & 0 & 0 & 0 & d_{15} & 0 \\ 
    0 & 0 & 0 & d_{24} & 0 & 0 \\ 
    d_{31} & d_{32} & d_{33} & 0 & 0 & 0 
    \end{pmatrix}
    ...
    d_{15} \sigma_{31} \\ 
    d_{24} \sigma_{23} \\ 
    d_{31} \sigma_{11} + d_{32} \sigma_{22} + d_{33} \sigma_{33} 
    \end{pmatrix}
    \]

    <p>
      By evaluating the resulting matrix we can determine which specific stresses generate charge across each direction:
    </p>

    <ul>
      <li><strong>Charge along axis $x_1$ ($Q_1 = d_{15} \sigma_{31}$):</strong> Generated by shear stress $\sigma_{31}$</li>

      <li><strong>Charge along axis $x_2$ ($Q_2 = d_{24} \sigma_{23}$):</strong> Generated by shear stress $\sigma_{23}$</li>

      <li><strong>Charge along axis $x_3$ ($Q_3 = d_{31} \sigma_{11} + d_{32} \sigma_{22} + d_{33} \sigma_{33}$):</strong> Generated by all three normal stresses</li>
    </ul>

    <p>
      In the majority of tactile sensing applications, we consider a <strong>uniaxial force</strong>: a compressive force $F$ is applied along axis 3 ($\sigma_{33} \propto F$), while all other stress components are zero. In this configuration, charge is accumulated only across the top and bottom electrodes on axis 3 ($Q = Q_3 = d_{33} F$). Substituting this charge $Q$ into the capacitor relationship $V = \frac{Q}{C}$, we find the simplified formula seen earlier:
    </p>
...
An example application of the piezoelectric technology is its integration into an **endoscopic grasper** to restore tactile feedback to the surgeon.

The design utilizes a multi-layered sensing unit integrated into the grasper's jaw. When the surgeon grips an object, the mechanical stress is converted into an electrical signal that indicates the magnitude and the position of the contact force.
...
<details class="quiz-details" markdown="1">
  ...
  <div class="quiz-question-text">
    A piezoelectric sensor ($d = 20\text{ pC/N}$, static capacitance $C = 2\text{ nF}$) is subjected to an unknown normal force $F(t)$. The output voltage $V(t)$ recorded across its electrodes is shown below:
  </div>

  <figure style="text-align: center; margin: 15px 0;">
    <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_ex.png' }}"
         width="480px"
         alt="Oscilloscope trace of output potential V(t)">
  </figure>

  <div class="quiz-question-text">
    1) Based on the voltage curve $V(t)$ seen on the oscilloscope, which applied force $F(t)$ produced this output? (single answer possible)
  </div>

  <form id="piezo-dyn-q1">

  <div style="margin-bottom: 20px;">
    <div style="display: flex; align-items: flex-start; gap: 8px;">
      <input type="radio" name="piezo-dyn-q1" value="option1" id="opt1" style="margin-top: 3px;">
      <label for="opt1">
        Option A: $F(t)$ mirrors the voltage output curve.
      </label>
    </div>
    <div style="text-align: center; margin-top: 8px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_ex_A.png' }}"
           width="360px"
           alt="Option A Force Profile">
    </div>
  </div>

  <div style="margin-bottom: 20px;">
    <div style="display: flex; align-items: flex-start; gap: 8px;">
      <input type="radio" name="piezo-dyn-q1" value="option2" id="opt2" style="margin-top: 3px;">
      <label for="opt2">
        Option B: $F(t)$ is a step force applied at $t = 1.0\text{s}$ and released at $t = 3.5\text{s}$.
      </label>
    </div>
    <div style="text-align: center; margin-top: 8px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_ex_B.png' }}"
           width="360px"
           alt="Option B Force Profile">
    </div>
  </div>

  <div style="margin-bottom: 20px;">
    <div style="display: flex; align-items: flex-start; gap: 8px;">
      <input type="radio" name="piezo-dyn-q1" value="option3" id="opt3" style="margin-top: 3px;">
      <label for="opt3">
        Option C: $F(t)$ is first a positive force plateau held from $t = 1.0\text{s}$ to $t = 2.5\text{s}$, followed by an opposite negative force plateau held held from $t = 3.5\text{s}$ to $t = 5.0\text{s}$.
      </label>
    </div>
    <div style="text-align: center; margin-top: 8px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_ex_C.png' }}"
           width="360px"
           alt="Option C Force Profile">
    </div>
  </div>

  <div style="margin-bottom: 20px;">
    <div style="display: flex; align-items: flex-start; gap: 8px;">
      <input type="radio" name="piezo-dyn-q1" value="option4" id="opt4" style="margin-top: 3px;">
      <label for="opt4">
        Option D: $F(t)$ is a continuous sinusoidal force acting between $t = 1.0\text{s}$ and $t = 3.5\text{s}$.
      </label>
    </div>
    <div style="text-align: center; margin-top: 8px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/piezo_ex_D.png' }}"
           width="360px"
           alt="Option D Force Profile">
    </div>
  </div>
  ...
  <p id="piezo-dyn-q1-feedback"></p>
  </form>

  <hr style="margin: 20px 0; border: 0; border-top: 1px solid #ccc;">

  <div class="quiz-question-text">
    2) Still consider the voltage curve from above. Using the peak potential $V_{peak} = +2.0\text{ V}$, calculate the magnitude of the applied normal force $F$. (single answer possible)
  </div>

  <form id="piezo-dyn-q2">

  <input type="radio" name="piezo-dyn-q2" value="option1">
  $F = 20\text{ N}$ <br>

  <input type="radio" name="piezo-dyn-q2" value="option3">
  $F = 2\text{ N}$ <br>

  <input type="radio" name="piezo-dyn-q2" value="option4">
  $F = 0.2\text{ N}$ <br>

  <input type="radio" name="piezo-dyn-q2" value="option2">
  $F = 200\text{ N}$ <br><br>
  ...
  <button type="button" onclick="checkMultipleTrueFalseMapped(
    ...
    'option2',
    {
      option1: '$F = 20\\text{ N}$ is wrong. From $V = \\frac{d}{C} F$, we find to $F = \\frac{V \\cdot C}{d}$. We then have $F = \\frac{2.0\\text{ V} \\cdot 2 \\times 10^{-9}\\text{ F}}{20 \\times 10^{-12}\\text{ C/N}} = 200\\text{ N}$.',
      option2: ' ',
      option3: '$F = 2\\text{ N}$ is wrong. From $V = \\frac{d}{C} F$, we find to $F = \\frac{V \\cdot C}{d}$. We then have $F = \\frac{2.0\\text{ V} \\cdot 2 \\times 10^{-9}\\text{ F}}{20 \\times 10^{-12}\\text{ C/N}} = 200\\text{ N}$.',
      option4: '$F = 0.2\\text{ N}$ is wrong. From $V = \\frac{d}{C} F$, we find to $F = \\frac{V \\cdot C}{d}$. We then have $F = \\frac{2.0\\text{ V} \\cdot 2 \\times 10^{-9}\\text{ F}}{20 \\times 10^{-12}\\text{ C/N}} = 200\\text{ N}$.'
    }
  )">
    Check Answer
  </button>

  <p id="piezo-dyn-q2-feedback"></p>
  </form>

  <hr style="margin: 20px 0; border: 0; border-top: 1px solid #ccc;">

<div class="quiz-question-text">
  3) Consider the dynamic sensing limitations of piezoelectric sensors, in which of the following robotics scenarios would this type of sensor be a smart choice? (multiple answers possible)
</div>

<form id="piezo-dyn-q3">
  <input type="checkbox" name="piezo-dyn-q3" value="option1">
  Detecting the start of object slip during robotic manipulation.<br>

  <input type="checkbox" name="piezo-dyn-q3" value="option2">
  Measuring the net weight of an item held stationary in a warehouse gripper over several minutes.<br>

  <input type="checkbox" name="piezo-dyn-q3" value="option3">
  Measuring surface texture variations as a tactile finger moves across a material.<br>

  <input type="checkbox" name="piezo-dyn-q3" value="option4">
  Measuring the force intensity of a grip during object slip.<br><br>

  <button type="button" onclick="checkMultipleAnswersMapped(
    'piezo-dyn-q3',
    ['option1', 'option3'],
    {
      option1: 'A) Correct. Slipping generates dynamic forces that piezoelectric sensors are good at capturing.',
      option2: 'B) Incorrect. Static weight measurement over time causes the generated charge to decay to zero, making it inaccurate for long-term holds.',
      option3: 'C) Correct. Surface scanning produces continuous vibrations that piezoelectric sensors can measure.',
      option4: 'D) Incorrect. Piezoelectric sensors cannot accurately measure grip force intensity because the charge decays and the output voltage does not vary linearly with static force.'
    }
  )">
    Check Answer
  </button>

  <p id="piezo-dyn-q3-feedback"></p>
</form>
...
<details class="quiz-details" markdown="1">
  ...
  <!-- QUESTION 1 -->
  <div class="quiz-question-text">
    1) Which advantage distinguishes optical tactile sensors (Type A and Type B) from resistive, capacitive and piezoelectric sensors?
    (single answer possible)
  </div>

  <form id="opt-q3">
    <input type="radio" name="opt-q3" value="option1">
    They can measure static loads without requiring any power supply.<br>

    <input type="radio" name="opt-q3" value="option2">
    They are immune to electromagnetic interference.<br>

    <input type="radio" name="opt-q3" value="option3">
    They do not require any flexible membrane or mechanical deformation layer.<br>

    <input type="radio" name="opt-q3" value="option4">
    Their output signal requires no electronic amplification or conversion.<br><br>

    <button type="button" onclick="checkMultipleTrueFalseMapped(
      'opt-q3',
      'option2',
      {
        option2: 'Because the sensing relies on optical light paths and scattering rather than electrical conductors, they do not react to electromagnetic interference.',
        option1: 'Optical sensors require active electrical components (LED emitters and cameras) to operate.',
        option3: 'Both Type A and Type B require deformable elastic membranes or rubber sheets with feelers to interact with the light path.',
        option4: 'Photodiodes and CCD cameras output currents/images that still require amplification/conversion into usable signals.'
      }
    )">
      Check Answer
    </button>
    <p id="opt-q3-feedback"></p>
  </form>

  <!-- QUESTION 2 -->
  <div class="quiz-question-text">
    2) The plot below shows the measured output voltage ($V_{out}$) versus applied force ($F$) for three identical light intensity optical sensors (Type A), each equipped with a cover membrane of different material stiffness (soft, intermediate, stiff).<br><br>
    Which curve corresponds to the <strong>soft membrane</strong>?
  </div>

  <div style="text-align: center; margin: 15px 0;">
    <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/optical_stiffness_curves.png' }}"
         width="460px"
         alt="Transfer characteristic curves for different membrane stiffness">
  </div>

  <form id="opt-q2">
    <input type="radio" name="opt-q2" value="optionA">
    Curve A<br>
    
    <input type="radio" name="opt-q2" value="optionB">
    Curve B<br>
    
    <input type="radio" name="opt-q2" value="optionC">
    Curve C<br><br>
    ...
    <button type="button" onclick="checkMultipleTrueFalseMapped(
      ...
      'optionA',
      {
        optionA: 'A softer membrane deforms easily under small loads (high sensitivity / steep initial slope), but the piston fully obstructs the light path early, causing the voltage to saturate quickly at $V_{max}$.',
        optionB: 'Curve B represents an intermediate stiffness offering a compromise between sensitivity and dynamic range.',
        optionC: 'Curve C represents a stiff membrane. It resists deformation, requiring much larger forces to move the piston, resulting in low sensitivity to light touches.'
      }
    )">
      Check Answer
    </button>
    <p id="opt-q2-feedback"></p>
  </form>

  <!-- QUESTION 3 -->
  <div class="quiz-question-text">
    3) A robotic gripper needs to handle small electronic components requiring contact forces between $0.2\text{ N}$ and $1.0\text{ N}$, as well as heavier metal parts requiring grip forces up to $8.5\text{ N}$<br><br>
    Based on the previous transfer curves, which membrane would be suitable of measuring across this force range without losing sensitivity at high loads?
  </div>

  <form id="opt-q2-followup">
    <input type="radio" name="opt-q2-followup" value="optA">
    Curve A<br>
    
    <input type="radio" name="opt-q2-followup" value="optB">
    Curve B<br>
    
    <input type="radio" name="opt-q2-followup" value="optC">
    Curve C<br><br>
    ...
    <button type="button" onclick="checkMultipleTrueFalseMapped(
      ...
      'optC',
      {
        optC: 'Curve C (stiff membrane) maintains a non-zero slope all the way past $8.5$ N without flattening out into saturation. While it sacrifices sensitivity at very low forces, it provides the broad range needed to measure also heavier loads.',
        optA: 'Curve A (soft membrane) completely saturates around $4$ N, meaning any force between $4$ N and $8.5$ N will almost output the identical maximum voltage ($V_{max}$), making it impossible to measure heavier parts.',
        optB: 'Curve B (intermediate membrane) begins to saturate around $6$ N to $7$ N, losing responsiveness before reaching the required $8.5$ N limit.'
      }
    )">
      Check Answer
    </button>
    <p id="opt-q2-followup-feedback"></p>
  </form>

  <!-- QUESTION 4 -->
  <div class="quiz-question-text">
    4) The three images below show the light patches recorded by the CCD sensor of a hemispherical FTIR robotic fingertip, similar to the one seen in the course, under three different contact states.<br><br>
    Select the correct applied force state for each monitor view:
  </div>

  <div style="display: flex; gap: 15px; justify-content: space-between; flex-wrap: wrap; margin-bottom: 20px;">
    
    <!-- State 1: Light Touch -->
    <div style="flex: 1; min-width: 200px; text-align: center; border: 1px solid #ccc; padding: 10px; border-radius: 6px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/optical_ex_monitor1.png' }}"
           width="100%" style="max-width: 170px; display: block; margin: 0 auto 10px auto;"
           alt="Monitor View 1">
      <select class="answer" data-answer="light" style="width: 100%; padding: 6px; font-size: 12px;">
        <option value="">-- Choose State --</option>
        <option value="normal">High normal force $+F_z$</option>
        <option value="shear">Moderate normal force $+F_z$ + rightward shear force $F_x$</option>
        <option value="light">Light normal force $+F_z$</option>
      </select>
    </div>

    <!-- State 2: High Normal Force -->
    <div style="flex: 1; min-width: 200px; text-align: center; border: 1px solid #ccc; padding: 10px; border-radius: 6px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/optical_ex_monitor2.png' }}"
           width="100%" style="max-width: 170px; display: block; margin: 0 auto 10px auto;"
           alt="Monitor View 2">
      <select class="answer" data-answer="normal" style="width: 100%; padding: 6px; font-size: 12px;">
        <option value="">-- Choose State --</option>
        <option value="normal">High normal force $+F_z$</option>
        <option value="shear">Moderate normal force $+F_z$ + rightward shear force $F_x$</option>
        <option value="light">Light normal force $+F_z$</option>
    ...
    <!-- State 3: Normal + Rightward Shear -->
    <div style="flex: 1; min-width: 200px; text-align: center; border: 1px solid #ccc; padding: 10px; border-radius: 6px;">
      <img src="{{ site.baseurl }}{{ '/assets/images/tactile_perception/optical_ex_monitor3.png' }}"
           width="100%" style="max-width: 170px; display: block; margin: 0 auto 10px auto;"
           alt="Monitor View 3">
      <select class="answer" data-answer="shear" style="width: 100%; padding: 6px; font-size: 12px;">
        <option value="">-- Choose State --</option>
        <option value="normal">High normal force $F_z$</option>
        <option value="shear">Moderate normal force $F_z$ + rightward shear force $F_x$</option>
        <option value="light">Light normal force $F_z$</option>
  ...
  <button type="button" onclick="checkDropdownAnswers('optical-match-feedback')">Check Matching</button>
  <p id="optical-match-feedback" style="margin-top: 8px; font-weight: bold;"></p>
