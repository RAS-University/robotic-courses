...
# =========================
# RRT + Path Shortcutting (animated, clean aesthetic)
# Produces: rrt_shortcut.mp4 (or .gif fallback)
# =========================
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle, Circle
...
if len(path_idx) < 2:
    print("No RRT path to shortcut. Skipping shortcut animation.")
else:
    # --------- Utilities ----------
    ...
    def try_shortcut(poly, i, j):
        """Replace poly[i:j+1] with a direct connection if collision-free."""
        if j <= i + 1:
            return None
        a, b = poly[i], poly[j]
        if visible_segment(a, b):
            return poly[:i+1] + [b] + poly[j+1:]
        return None

    # Copy path from RRT
    path_poly = [nodes[k] for k in path_idx]
    initial_len = path_length(path_poly)

    # --------- Precompute accepted shortcut states ----------
    N_ITERATIONS = 200
    rng_shortcut = np.random.default_rng(123)
    current = path_poly[:]
    states = [current]
    ...
    for _ in range(N_ITERATIONS):
        if len(current) < 3:
            break
        i = int(rng_shortcut.integers(0, len(current)-2))
        j = int(rng_shortcut.integers(i+2, len(current)))
        new_poly = try_shortcut(current, i, j)
        if new_poly is not None:
            current = new_poly
            states.append(current)

    final_len = path_length(states[-1])
    print(f"Accepted {len(states)-1} shortcuts; length {initial_len:.2f} → {final_len:.2f}")

    # --------- Visualization ----------
    fig_sc, ax_sc = plt.subplots(figsize=(GRID_W * CELL_PX / 110, GRID_H * CELL_PX / 110), dpi=110)
    ax_sc.set_aspect('equal')
    ax_sc.set_xlim(0, GRID_W)
    ax_sc.set_ylim(GRID_H, 0)
    ax_sc.set_xticks([]); ax_sc.set_yticks([])
    ax_sc.set_title("Path Shortcutting (Post-Processing)", pad=10, fontsize=11)
     
    # Maze
    for y in range(GRID_H):
        for x in range(GRID_W):
            if grid[y, x] == 1:
                ax_sc.add_patch(Rectangle((x, y), 1, 1, linewidth=0, facecolor='black', zorder=0))

    # Start / Goal
    sx, sy = start_world
    gx, gy = goal_world
    ax_sc.add_patch(Circle((sx, sy), 0.28, facecolor="#13adfa", edgecolor='black', lw=0.7, zorder=4))
    ax_sc.add_patch(Circle((gx, gy), 0.28, facecolor="#0feb55", edgecolor='black', lw=0.7, zorder=4))

    # Original RRT path (light gray)
    if len(path_poly) >= 2:
        xs0, ys0 = zip(*path_poly)
        ax_sc.plot(xs0, ys0, lw=2.0, color="#aaaaaa", alpha=0.5, zorder=2)

    # Dynamic current (orange) path line
    path_line, = ax_sc.plot([], [], lw=3.0, color="#ff7f0e", alpha=1.0, zorder=3)

    # Text overlay for iteration counter (small, bottom-left)
    text_iter = ax_sc.text(0.5, GRID_H - 0.5, "", color="#666", fontsize=8)
    # ax_sc.legend(loc="lower right", frameon=True, fontsize=8)  
    # --------- Animation ----------
    def init_shortcut():
        path_line.set_data([], [])
        text_iter.set_text("")
    ...
    def animate_shortcut(frame):
        poly = states[min(frame, len(states)-1)]
        xs, ys = zip(*poly)
        path_line.set_data(xs, ys)
        # text_iter.set_text(f"Shortcut iteration: {frame}/{len(states)-1}")
        return [path_line, text_iter]

    # Slower, smoother pace
    ANIM_INTERVAL_MS = 500  # ~11 fps (visually calm)
    anim_sc = FuncAnimation(
        fig_sc, animate_shortcut, init_func=init_shortcut,
        frames=max(1, len(states)), interval=ANIM_INTERVAL_MS,
        blit=False, repeat=False
    )
    ...
    try:
        ...
        writer_sc = FFMpegWriter(fps=max(1, int(1000/ANIM_INTERVAL_MS)))
        anim_sc.save("rrt_shortcut.mp4", writer=writer_sc, dpi=110)
        print("Saved rrt_shortcut.mp4")
        saved_sc = True
    ...
    if not saved_sc:
        try:
            ...
            writer_sc = PillowWriter(fps=max(1, int(1000/ANIM_INTERVAL_MS)))
            anim_sc.save("rrt_shortcut.gif", writer=writer_sc, dpi=110,
                         savefig_kwargs={"facecolor": "white"})
            print("Saved rrt_shortcut.gif")
            saved_sc = True
