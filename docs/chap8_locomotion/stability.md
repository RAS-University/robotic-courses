...
*This page continues directly from [8.1 Foundation of legged locomotion](#), where we met the planar two-link leg, its kinematics and dynamics, and the classification of gaits. Several figures below are adapted from the **Legged Robots** course at EPFL, Lecture 2 ("Gaits, Models, Stability Criteria, and Locomotion Metrics") by Pr. Auke Ijspeert; full credit is given in the [Credits](#credits) section at the end of the page.*
...
<!-- VIDEO SLOT: an opening clip motivating this page, e.g. a humanoid recovering from a push (ZMP / capture-point style balance control), or a robot falling because a stability criterion was violated. One reasonable candidate found while researching this page: "Humanoid Robot Push Recovery Experiments" (Sarcos Primus, CMU, https://www.youtube.com/watch?v=fxCzWX-uzjk), footage is dated (2010) so you may prefer a more recent one; leaving this open for you to choose. -->
...
### 1. Why do we need models and stability criteria?
...
### 3. Center of Pressure and the Zero Moment Point
...
In Section 2 the legs were idealized as *points*, so the support polygon was the convex hull of a handful of contact points. A biped with real, flat feet is different: each foot touches the ground over a whole **surface**, not a point. The support polygon is therefore the convex hull of the entire **contact area**:
...
Stand still ($\ddot{x} = 0$) and the ZMP sits exactly under your CoM. Accelerate forward ($\ddot{x} > 0$) and the ZMP shifts *backward*, toward your heels. Brake ($\ddot{x} < 0$) and it shifts forward, toward your toes, which is exactly why you pitch onto your toes when you stop abruptly. We will meet this same equation again in Section 4, where it turns out to be the LIP model in disguise.
...
Two honest caveats. First, tipping is not the same as falling: a robot whose ZMP briefly exits the support polygon may still recover by taking a step, which is exactly what the capture point of Section 5 formalizes. Second, this whole criterion presumes a foot is on the ground, so it says nothing at all about the flight phase of a running or hopping robot; Section 7 provides the right tool there.
...
This is the essential difference between the static criterion of Section 2 and the dynamic one here. Static stability demanded that the CoM projection stay inside the support polygon. Dynamic stability makes no such demand: the CoM projection may be well outside the feet, as long as the robot is accelerating in the right way to keep the *ZMP* inside.
...
<figure style="margin: 1.5rem auto; text-align: center;">
  ...
  <figcaption style="max-width: 700px; margin: 0.5rem auto; font-size: 0.9rem; color: #4b5563;">
    <strong>Figure 7: The CoM projection may leave the support polygon; the ZMP may not.</strong> As the body leans and accelerates forward, its CoM projection (orange) crosses outside the foot, which would be an immediate fall by the static criterion of Section 2. The ZMP (purple) stays inside the foot throughout, so no tipping moment develops and the motion is dynamically stable.
  </figcaption>
...
<button
  ...
  onclick="checkMCQ(
    ...
    'Incorrect. Re-read the boxed stability rule in Section 3.2.'
  )">
  Check answer
</button>
...
### 4. The Linear Inverted Pendulum (LIP), in depth
...
Kajita and Tani (1991) proposed a simplification that removes exactly that difficulty. It trades a little realism for an **exact, closed-form solution**, and that solution is what makes real-time footstep planning possible; it is also the model from which the capture point of Section 5 is derived.
...
#### 4.2 Derivation of the equation of motion
...
**Step 3: convert the angle into a position.** This is the step that produces the linearity. From the geometry of Section 4.1, dividing $x = l\sin\alpha$ by $z\_0 = l\cos\alpha$ makes the unknown length $l$ cancel:
...
### 5. The Spring-Loaded Inverted Pendulum (SLIP), in depth
...
#### 5.1 Predictive validity and biological relevance
...
#### 5.2 Model definition and notation
...
#### 5.3 Modelling assumptions and their implications
...
#### 5.4 Equations of motion: Newton-Euler formulation
...
#### 5.5 Hybrid dynamics and phase-transition conditions
...
#### 5.6 Design parameters and degrees of freedom
...
#### 5.7 Passive self-stabilisation and parameter sensitivity
...
*(How the "steps to fall" test is made quantitative, rather than just counted, is the subject of Section 7.)*
...
### 6. Capture point and the Divergent Component of Motion (DCM)

We now return to walking, and to the LIP. The ZMP of Section 3 tells you whether you are *currently* tipping, and the LIP of Section 4 tells you how the CoM will move once a foot is planted. Neither answers the question a balance controller actually faces after a push: **where should I put the next foot?** This section derives the answer, and it falls directly out of the LIP's closed-form solution.

#### 6.1 Capturability: the qualitative notion
...
#### 6.2 Change of variables: decoupling the LIP dynamics

Recall the 2D LIP equation of motion derived in Section 4, with the support point at $x\_{base}$:
...
The instability we identified in Section 4, the growing exponential $Be^{\omega t}$, has now been cleanly separated out. All of the divergence lives in $\xi$, and none of it lives in $x$. That is what makes this change of variables so useful: instead of controlling a second-order unstable system, we need only steer one scalar quantity.

#### 6.3 The instantaneous capture point
...
#### 6.4 Terminology and further reading
...
### 7. Poincaré maps and return-map analysis
...
#### 7.1 Poincaré maps: reducing a continuous flow to a discrete map
...
#### 7.2 The return map and the choice of section
...
#### 7.3 Conditions for a stable periodic gait
...
#### 7.4 Application: the return map of the SLIP model

We can now make quantitative the "steps to fall" experiment of Section 5.7.

For the SLIP model, the return map $y\_{i+1}(y\_i)$ is a function defined for **a given set of open parameters** $(v\_0, \alpha\_0, k)$. It is **obtained numerically**, by integrating the differential equations of the SLIP model derived in Section 5.4 through one complete step, from one apex to the next. Once computed, it can be used to generate the entire series of apex heights from any initial condition $y\_0$.
...
### 8. Locomotion performance metrics
...
## Numerical practical (coming later)

<div class="coming-soon" markdown="1">

A programming exercise is planned for this page, most likely simulating the LIP and/or SLIP model in Python (numerically integrating the SLIP stance/flight equations, computing a return map, and/or implementing a simple LIP-based footstep planner using the DCM rule derived in Section 5), in the same style as the two practicals in 8.1. It will be added here once ready; the pen-and-paper exercises above cover the same core computations by hand in the meantime.

</div>

<!-- IMAGE/VIDEO SLOT: consider a short clip or GIF here once the practical is ready, e.g. a simulated LIP walker doing DCM-based footstep planning, or a SLIP hopper converging to its periodic gait from a perturbed initial condition. -->
